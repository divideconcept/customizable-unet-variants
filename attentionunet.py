...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, conv_per_block, kernel_size, batch_norm,
        ...
        self.attention = Attn(out_channels)
    ...
    def forward(self, from_down, from_up):
        ...
        if not self.add_merging:
            x = torch.cat((from_up, self.attention(from_up, from_down)), 1)
        else:
            x = from_up + from_down
        x = self.block(x)
        return x
...
class AttentionUNet(nn.Module):
    """ `AttentionUNet` class is based on https://arxiv.org/pdf/1804.03999.pdf
    Attention UNet is a convolutional encoder-decoder neural network with attention mechanism.
    
    Default parameters correspond to the original Attention UNet.
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_channels=32,
